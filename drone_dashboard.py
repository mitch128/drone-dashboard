...
#######################################
def make_dummy_data():
    # We'll simulate 3 drones (each with id and type) for a 60-sec simulation in 3D (x, y, z)
    np.random.seed(42)  # reproducible data
    timestamps = np.arange(0, 61)  # 0 to 60 seconds
    rows = []
...
#######################################
# 5. Map Overlay Function
#######################################
def create_map_overlay(t):
    # Create a map centered around the friendly positions
    m = folium.Map(location=[0, 0], zoom_start=15)

    # Add friendly positions to the map
    for name, (x, y, _) in infantry_positions.items():
        folium.Marker([y, x], popup=name, icon=folium.Icon(color='blue', icon='info-sign')).add_to(m)

    # Add drone positions to the map
    current_frame = df[df.time == t]
    for _, row in current_frame.iterrows():
        drone_type = row['type']
        color = 'red' if drone_type == 'Shahed' else 'green' if drone_type == 'Recon' else 'blue'
        folium.Marker([row.y, row.x], popup=f"{row['id']} ({drone_type})", icon=folium.Icon(color=color)).add_to(m)

    return m

#######################################
# 6. Streamlit UI – Combined Dashboard (3D, 2D, and Map)
#######################################
...
# Layout: three side-by-side panels for 3D, 2D, and Map views; summary and event logs are below.
col_3d, col_2d, col_map = st.columns(3)
...
#######################################
# 7. Simulation / Slider Update Handling
#######################################
if play:
    ...
    map_overlay = create_map_overlay(t_slider)
    radar_placeholder_3d.pyplot(fig3d)
    radar_placeholder_2d.pyplot(fig2d)
    map_placeholder.folium_static(map_overlay)
    all_drone_info = {}
